...
class MusicPlayerScreen : Screen(TITLE), ClientPlaybackHandler.GuiListener {
    ...
    private var queueTotal = -1
    private var queueHasMore = false
    ...
    private fun addQueueWidgets() {
        ...
        val clearButton = Button.builder(McText.translatable("screen.moemusic.queue.clear")) {
            ...
            rebuildScreenWidgets()
        }.pos(margin + 64, contentY).size(60, 16).build()
    ...
    private fun renderNowPlaying(context: GuiGraphicsExtractor, mouseX: Int, mouseY: Int) {
        val ctx = ClientPlaybackHandler.currentContext
        val quickAddLayout = quickAddLayout()
        val textX = margin + 6
        ...
        context.text(font, McText.literal(volumeLabel), textX, volumeY - font.lineHeight - 2, dimCol, true)
        ...
        context.text(font, McText.literal(timeStr), textX, timeY, dimCol, true)
        when {
            playbackError != null ->
                context.text(font, McText.literal("✗ $playbackError"), textX, timeY + lineH + 6, errCol, true)

            playbackSuccess != null ->
                context.text(font, McText.literal("✓ $playbackSuccess"), textX, timeY + lineH + 6, successCol, true)
        }
    ...
    private fun renderSearch(context: GuiGraphicsExtractor, mouseX: Int, mouseY: Int) {
        val sources = searchableSources()
        if (sources.isEmpty()) {
            context.text(
                font,
            ...
            context.text(font, McText.literal(emptyMessage), margin + 6, listY + 4, dimCol, true)
        } else {
            renderSelectionList(
                context = context,
                mouseX = mouseX,
    ...
    private fun renderSearchSourceDropdown(context: GuiGraphicsExtractor, mouseX: Int, mouseY: Int) {
        val sources = searchableSources()
    ...
    private fun renderQueue(context: GuiGraphicsExtractor, mouseX: Int, mouseY: Int) {
    ...
    private fun renderInlineSegments(
        ...
        for (segment in segments) {
            ...
            if (drawX + segmentWidth <= endX) {
                context.text(font, McText.literal(segment.text), drawX, y, segment.color, false)
                drawX += segmentWidth
                continue
            }
            ...
            if (truncated.isNotEmpty()) {
                context.text(font, McText.literal(truncated), drawX, y, segment.color, false)
            }
            return
        }
    }
    ...
    override fun mouseDragged(event: MouseButtonEvent, dx: Double, dy: Double): Boolean {
        listControllerDragging?.let { variant ->
            updateListControllerDrag(variant, event.y().toInt())
            return true
        }
        if (currentTab == Tab.NOW_PLAYING && volumeBarDragging) {
            handleVolumeClick(event.x().toInt(), event.y().toInt(), pressed = false)
            return true
        }
        if (currentTab == Tab.NOW_PLAYING && seekBarDragging) {
            handleSeekClick(event.x().toInt(), event.y().toInt(), pressed = false)
            return true
        }
    ...
    override fun mouseScrolled(mouseX: Double, mouseY: Double, horizAmount: Double, vertAmount: Double): Boolean {
        val delta = if (vertAmount > 0) -1 else 1
        when (currentTab) {
            Tab.SEARCH -> {
    ...
    override fun keyPressed(event: KeyEvent): Boolean {
        ...
        if (isConfirmKey(event.key()) && currentTab == Tab.SEARCH && searchEditBox?.isFocused == true) {
    ...
    private fun renderRowActionMenuBackground(context: GuiGraphicsExtractor) {
