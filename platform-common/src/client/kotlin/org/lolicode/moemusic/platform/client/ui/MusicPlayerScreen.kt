...
class MusicPlayerScreen : Screen(TITLE), ClientPlaybackHandler.GuiListener {
    ...
    private fun addNowPlayingWidgets() {
        ...
        addRenderableWidget(button(modeButtonX, quickAddLayout.addBoxY, modeButtonW, 16, McText.literal(addModeLabel(addIdentifierMode))) {
            addIdentifierMode = nextAddMode(addIdentifierMode)
            rebuildScreenWidgets()
        })

        addRenderableWidget(button(addButtonX, quickAddLayout.addBoxY, addButtonW, 16, McText.translatable("screen.moemusic.quick_add.add")) {
        ...
        addRenderableWidget(button(playPauseButtonX, btnY, btnW, btnH, McText.translatable(playLabelKey)) {
            playbackError = null
        ...
        addRenderableWidget(button(skipButtonX, btnY, btnW, btnH, McText.translatable("screen.moemusic.control.skip")) {
            playbackError = null
            playbackSuccess = null
            pendingPlaybackControlRequestId = ClientPlaybackHandler.sendPlaybackControl(PlaybackControlAction.SKIP)
        })

        addRenderableWidget(button(stopButtonX, btnY, btnW, btnH, McText.translatable("screen.moemusic.control.stop")) {
            playbackError = null
            playbackSuccess = null
    ...
    private fun addQueueWidgets() {
        ...
        addRenderableWidget(button(margin, contentY, 60, 16, McText.translatable("screen.moemusic.queue.refresh")) {
            queueError = null
            queueSuccess = null
            pendingQueueRequestId = ClientPlaybackHandler.sendQueueRequest()
        })
