...
class MusicPlayerScreen : Screen(TITLE), ClientPlaybackHandler.GuiListener {
    ...
    private fun addTabButtons() {
        Tab.entries.forEachIndexed { i, tab ->
            ...
            addRenderableWidget(button(margin + i * topButtonW, margin, topButtonW - 1, tabBarH - 2, McText.translatable(labelKey)) {
                searchSourceDropdownOpen = false
                rowActionMenu = null
                rowActionMenuLayout = null
                currentTab = tab
        ...
        addRenderableWidget(
            button(configButtonX, margin, topButtonW - 1, tabBarH - 2, McText.translatable("screen.moemusic.config.button")) {
    ...
    private fun addNowPlayingWidgets() {
        ...
        val desiredModeButtonW = listOf(
            TrackAddMode.NORMAL,
            TrackAddMode.SKIP_AUTOPLAY,
            TrackAddMode.PLAY_NOW,
        ).maxOf { font.width(addModeLabel(it)) } + 10
        val maxModeButtonW = (addPanelW - addButtonW - addButtonGap * 2 - 60).coerceAtLeast(82)
        val modeButtonW = desiredModeButtonW.coerceAtLeast(82).coerceAtMost(maxModeButtonW)
        ...
        box.setHintCompat(McText.translatable("screen.moemusic.quick_add.hint"))
        box.value = addIdentifierInputText
        box.active = canQuickAdd
        addIdentifierEditBox = box
        addRenderableWidget(box)

        val modeButton = button(modeButtonX, quickAddLayout.addBoxY, modeButtonW, 16, McText.literal(addModeLabel(addIdentifierMode))) {
        ...
        val addButton = button(addButtonX, quickAddLayout.addBoxY, addButtonW, 16, McText.translatable("screen.moemusic.quick_add.add")) {
        ...
        val playPauseButton = button(playPauseButtonX, btnY, btnW, btnH, McText.translatable(playLabelKey)) {
            playbackError = null
            playbackSuccess = null
            playbackErrorFromInstanceLock = false
        ...
        val skipButton = button(skipButtonX, btnY, btnW, btnH, McText.translatable("screen.moemusic.control.skip")) {
            playbackError = null
            playbackSuccess = null
            playbackErrorFromInstanceLock = false
            playbackErrorFromLocalPlayback = false
        ...
        val stopButton = button(stopButtonX, btnY, btnW, btnH, McText.translatable("screen.moemusic.control.stop")) {
            playbackError = null
            playbackSuccess = null
            playbackErrorFromInstanceLock = false
            playbackErrorFromLocalPlayback = false
    ...
    private fun addSearchWidgets() {
        ...
        box.setHintCompat(McText.translatable("screen.moemusic.search.field_hint"))
        box.value = searchInputText
        box.active = canUseSearch
        searchEditBox = box
        addRenderableWidget(box)

        val searchButton = button(editX + editW + controlGap, contentY, searchBtnW, searchControlH, McText.translatable("screen.moemusic.search.button")) {
            searchSourceDropdownOpen = false
        ...
        val searchUpButton = button(controller.x, controller.upButtonY, controller.width, listControllerBtnH, McText.literal("▲")) {
        ...
        val searchDownButton = button(controller.x, controller.downButtonY, controller.width, listControllerBtnH, McText.literal("▼")) {
            setSearchAbsoluteOffset(currentSearchAbsoluteOffset() + 1)
        }
    ...
    private fun addQueueWidgets() {
        ...
        val refreshButton = button(margin, contentY, 60, 16, McText.translatable("screen.moemusic.queue.refresh")) {
            queueError = null
            queueSuccess = null
        ...
        val queueDownButton = button(controller.x, controller.downButtonY, controller.width, listControllerBtnH, McText.literal("▼")) {
    ...
    override fun renderBackground(poseStack: PoseStack) = Unit
    ...
    private fun renderSearch(context: GuiGraphics, mouseX: Int, mouseY: Int) {
        ...
        } else if (searchResults.isEmpty() && !searchSourceDropdownOpen) {
    ...
    private fun renderSearchSourceDropdown(context: GuiGraphics, mouseX: Int, mouseY: Int) {
        ...
        sources.forEachIndexed { index, source ->
            ...
            context.drawString(
                ...
                x + 4,
                centeredTextY(rowTop, searchDropdownRowH, font),
                color,
                false,
            )
        }
    ...
    private fun renderRowButton(
        ...
        val visibleLabel = truncateToWidth(label, width - 4, font)
        context.drawString(
            font,
            McText.literal(visibleLabel),
            x + (width - font.width(visibleLabel)) / 2,
            centeredTextY(y, btnRowH, font),
            buttonText,
            false,
        )
    }
    ...
    override fun keyPressed(keyCode: Int, scanCode: Int, modifiers: Int): Boolean {
        if (keyCode == InputConstants.KEY_ESCAPE && currentTab == Tab.SEARCH && searchSourceDropdownOpen) {
            searchSourceDropdownOpen = false
            rebuildScreenWidgets()
            return true
        }
