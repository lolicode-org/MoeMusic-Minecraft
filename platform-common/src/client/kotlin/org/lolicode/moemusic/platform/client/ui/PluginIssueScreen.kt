...
class PluginIssueScreen(
    ...
    private fun rebuildButtons() {
        ...
        addRenderableWidget(
            Button.builder(McText.translatable("screen.moemusic.plugin_issues.button.open_mods")) {
                openDirectory(modsDir)
            }.pos(width / 2 - 155, btnRow1Y).size(150, 20).build()
        )

        addRenderableWidget(
            Button.builder(McText.translatable("screen.moemusic.plugin_issues.button.open_plugins")) {
                openDirectory(pluginsDir)
            }.pos(width / 2 + 5, btnRow1Y).size(150, 20).build()
        )

        // Row 2: Action buttons
        addRenderableWidget(
            Button.builder(McText.translatable("screen.moemusic.plugin_issues.button.continue")) {
                onClose()
            }.pos(width / 2 - 155, btnRow2Y).size(150, 20).build()
        )

        addRenderableWidget(
            Button.builder(McText.translatable("screen.moemusic.plugin_issues.button.quit")) {
                minecraft.stop()
            }.pos(width / 2 + 5, btnRow2Y).size(150, 20).build()
        )
    }
    ...
    override fun extractRenderState(context: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        // Header
        context.centeredText(font, title, width / 2, 14, 0xFFFFFFFF.toInt())
        ...
        // Render cards with scissor
        context.enableScissor(cardLeft, listTop, cardRight + 12, listBottom)
        ...
        for (i in issues.indices) {
            ...
            if (itemBottom >= listTop && currentY <= listBottom) {
                ...
                context.text(font, item.tag, cardLeft + 8, textY, item.tagColor, false)
                val tagW = font.width(item.tag)
                context.text(font, item.title, cardLeft + 8 + tagW + 6, textY, 0xFFFFFFFF.toInt(), false)
                ...
                for (lineSplits in splitLines) {
                    for (seq in lineSplits) {
                        context.text(font, seq, cardLeft + 8, textY, 0xFFAAAAAA.toInt(), false)
    ...
    override fun mouseScrolled(mouseX: Double, mouseY: Double, horizAmount: Double, vertAmount: Double): Boolean {
        ...
        if (maxScroll > 0) {
            scrollAmount = (scrollAmount - vertAmount * 24.0).coerceIn(0.0, maxScroll.toDouble())
            return true
        }
