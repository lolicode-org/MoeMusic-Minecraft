...
object NowPlayingHud {
    ...
    private data class CoverTextures(
        val square: Identifier,
        val circular: Identifier,
    )
    ...
    private val logoCircularCache = ConcurrentHashMap<String, Identifier>()
    private val logoCircularLoadFailures = ConcurrentHashMap.newKeySet<String>()

    @Volatile
    private var logoSquareTexture: Identifier? = null
    ...
    private val MOD_LOGO_RESOURCE_ID = Identifier.fromNamespaceAndPath(MOD_ID, "icon.png")
    ...
    private fun renderCircularCover(
        ...
        val pose: Matrix3x2fStack = gfx.pose()
        pose.pushMatrix()
    ...
    private fun renderScaledText(
        ...
        val pose = gfx.pose()
        pose.pushMatrix()
        pose.translate(drawX.toFloat(), top.toFloat())
        pose.scale(scale, scale)
        gfx.text(font, McText.literal(truncated), 0, 0, color, shadow)
        pose.popMatrix()
    }
    ...
    private fun getLogoSquareTexture(): Identifier? {
        ...
        synchronized(logoTextureLock) {
            ...
            return try {
                ...
                val id = Identifier.fromNamespaceAndPath(MOD_ID, "cover/fallback_logo")
                var registered = false
                try {
                    Minecraft.getInstance().textureManager.register(
                        id,
                        DynamicTexture({ "$MOD_ID:cover/fallback_logo" }, normalized),
                    )
                    registered = true
                } finally {
    ...
    private fun getLogoCircularTexture(recordRingColorArgb: String, showCenterDot: Boolean): Identifier? {
        ...
        synchronized(logoTextureLock) {
            ...
            return try {
                ...
                val id = Identifier.fromNamespaceAndPath(MOD_ID, "cover/fallback_logo_record_$safeKey")
                var registered = false
                try {
                    Minecraft.getInstance().textureManager.register(
                        id,
                        DynamicTexture({ "$MOD_ID:cover/fallback_logo_record_$safeKey" }, circular),
                    )
                    registered = true
                } finally {
    ...
    private fun loadModLogoImage(): NativeImage =
        Minecraft.getInstance().resourceManager.open(MOD_LOGO_RESOURCE_ID).use { NativeImage.read(it) }
            ...
            try {
                ...
                Minecraft.getInstance().execute {
                    try {
                        ...
                        val safeKey = cacheKey.hashCode().toUInt().toString(16)
                        val squareId = Identifier.fromNamespaceAndPath("moemusic", "cover/$safeKey")
                        val circularId = Identifier.fromNamespaceAndPath("moemusic", "cover/${safeKey}_record")
                        Minecraft.getInstance().textureManager.register(
                            squareId,
                            DynamicTexture({ "moemusic:cover/$safeKey" }, normalized),
                        )
                        Minecraft.getInstance().textureManager.register(
                            circularId,
                            DynamicTexture({ "moemusic:cover/${safeKey}_record" }, circular),
                        )
                        synchronized(coverCacheLock) {
    ...
    private fun BufferedImage.toNativeImage(): NativeImage {
        ...
        for (y in 0 until height) {
            for (x in 0 until width) {
                nativeImage.setPixelABGR(x, y, ARGB.toABGR(argbPixels[index++]))
            }
        }
        return nativeImage
    }
            ...
            for (x in 0 until size) {
                ...
                val pixel = when {
                    ...
                    distance <= innerImageRadius &&
                        ...
                        scaledImage.getPixel(x - imageStart, y - imageStart)
