...
object NowPlayingHud {
    ...
    private val MOD_LOGO_RESOURCE_ID = ResourceLocation.fromNamespaceAndPath(MOD_ID, "icon.png")
    ...
    private fun getLogoSquareTexture(): ResourceLocation? {
        ...
        synchronized(logoTextureLock) {
            ...
            return try {
                ...
                val id = ResourceLocation.fromNamespaceAndPath(MOD_ID, "cover/fallback_logo")
    ...
    private fun getLogoCircularTexture(recordRingColorArgb: String, showCenterDot: Boolean): ResourceLocation? {
        ...
        synchronized(logoTextureLock) {
            ...
            return try {
                ...
                val id = ResourceLocation.fromNamespaceAndPath(MOD_ID, "cover/fallback_logo_record_$safeKey")
                var registered = false
                try {
                    Minecraft.getInstance().textureManager.register(
                        id,
            ...
            try {
                ...
                Minecraft.getInstance().execute {
                    try {
                        ...
                        val safeKey = cacheKey.hashCode().toUInt().toString(16)
                        val squareId = ResourceLocation.fromNamespaceAndPath("moemusic", "cover/$safeKey")
                        val circularId = ResourceLocation.fromNamespaceAndPath("moemusic", "cover/${safeKey}_record")
                        Minecraft.getInstance().textureManager.register(
                            squareId,
                            DynamicTexture(normalized),
                        )
    ...
    private fun BufferedImage.toNativeImage(): NativeImage {
        ...
        for (y in 0 until height) {
            for (x in 0 until width) {
                nativeImage.setPixelRGBA(x, y, FastColor.ABGR32.fromArgb32(argbPixels[index++]))
            }
        }
        return nativeImage
    }
