...
object MusicCommands {
    ...
    private fun requiresSkipPermission(): (CommandSourceStack) -> Boolean = { source ->
        PermissionResolver.hasPermission(source, PermissionNodes.QUEUE_CONTROL) ||
            (source.player != null && PermissionResolver.hasPermission(source, PermissionNodes.VOTE))
    }
    ...
    private fun sourceUser(source: CommandSourceStack) =
        source.player?.let(MinecraftUserRegistry::snapshot)

    private fun sourceLocale(source: CommandSourceStack): String =
        sourceLocale(source.player?.uuid)
    ...
    private fun applyCommandAction(
        ...
        style
            .withClickEvent(ClickEvent.RunCommand(action.command))
            .withHoverEvent(HoverEvent.ShowText(localizedComponent(source, action.hover)))
    }
    ...
    private fun applyHoverText(
        ...
        style.withHoverEvent(HoverEvent.ShowText(localizedComponent(source, hover, LocalizedChatRenderer.Tone.FAILURE)))
    }
        ...
        return McText.literal("§8[")
            ...
            .withStyle { style ->
                style
                    ...
                    .withHoverEvent(HoverEvent.ShowText(localizedComponent(source, hover)))
            }
    }
