 ...
 *   (broadcast).
 * - **C→S receiving**: registered via [C2SPacketReceiver]; decoded bytes are
...
class BadPacketsNetworkChannel(
    ...
    /** Register all C→S receivers. Bad Packets 0.4.x uses receiver registration as channel declaration. */
    ...
    private val identifierCache = ConcurrentHashMap<PacketId, ResourceLocation>()
    private fun PacketId.toIdentifier(): ResourceLocation =
        identifierCache.computeIfAbsent(this) { ResourceLocation(it.namespace, it.path) }
