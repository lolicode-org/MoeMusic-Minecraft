...
MoeMusic for Minecraft is a music player mod / plugin for Minecraft. It works on fabric / neoforge / forge / spigot / paper / velocity, supports multiple music sources, allowing players to request, search, skip, and manage tracks via in-game controls. When installed on a server, it can also coordinate a shared queue and keep playback in sync across connected clients.
