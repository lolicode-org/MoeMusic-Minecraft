...
val kotlinRuntimeModules = mapOf(
    "org.jetbrains.kotlin" to setOf(
        "kotlin-reflect",
        "kotlin-stdlib",
    ...
    "org.jetbrains.kotlinx" to setOf(
...
fun shouldEmbedInForgeDevMod(id: ComponentIdentifier): Boolean =
    when (id) {
        is ModuleComponentIdentifier -> id.group == "org.lolicode.moemusic" && id.module in sharedModuleNames
                || id.module in kotlinRuntimeModules[id.group].orEmpty()
...
val bundledKotlinRuntime = listOf(
    libs.kotlin.stdlib,
...
val unpackSharedRuntimeForDev by tasks.registering(Sync::class) {
    ...
    // This dev-only exploded classpath flattens several multi-release Kotlin jars into one directory.
    // JPMS descriptors and repeated resource files can legitimately collide there.
...
dependencies {
    ...
    compileOnly(libs.lwjgl.openal)

    compileOnly(libs.badpackets.forge)
    add(rawBadPacketsForge.name, libs.badpackets.forge)
    runtimeOnly(files(patchBadPacketsForgeForDev.flatMap { it.archiveFile }))

    bundledKotlinRuntime.forEach { dependency ->
        implementation(dependency)
        add(sharedRuntimeArtifacts.name, dependency)
        add(installShade.name, dependency)
    }
...
tasks.processResources {
    ...
    val replaceProperties = mapOf(
        ...
        "fml_loader_version_range" to "[${forgeVersion.substringAfterLast('-').substringBefore('.')},)",
    )
    ...
    from({ sharedAssetResources.map { zipTree(it) } }) {
        include("assets/moemusic/icon.png")
        eachFile { path = name }
...
tasks.withType<JavaExec>().configureEach {
    if (name in setOf("runClient", "runServer", "runGameTestServer", "runData")) {
        // Forge's launcher reflectively resolves BootstrapLauncher from the unnamed module on Java 17.
